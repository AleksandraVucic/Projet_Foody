...
WHEN (weekday(DateCom)=6) THEN "week-end"
ELSE "No info"
...
select NoFour, RefProd, PrixUnit,NomProd, max(PrixUnit) from foody.produit group by  RefProd having max(PrixUnit)>50;
...
Select p.RefProd, p.NomProd, f.* from foody.produit as p natural join foody.fournisseur as f;
#2.Afficher les informations des commandes du client "Lazy K Kountry Store"
Select cl.Societe, co.* from foody.commande as co natural join foody.client as cl  where cl.Societe= "Lazy K Kountry Store"  ;
...
Select m.NomMess,Count(co.NoCom) as 'le nombre de commande' from foody.commande as co NATURAL JOIN foody.messager as m group by NoMess;
...
Select m.NomMess,Count(co.NoCom) as 'le nombre de commande' from foody.commande as co JOIN foody.messager as m using(NoMess) group by NoMess;
